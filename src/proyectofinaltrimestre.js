db.futbolistaspremierleague.insertMany([
    ...
    },
    {
     ...
     db.futbolistaspremierleague.find( { "edad":{ $gt:25  }, "edad": { $lt: 29 } } )//Find que da los jugadores que están entre los 25 y 29 años
     db.futbolistaspremierleague.find( { "pie preferido": null } ) //Find para los valores de pie preferido que son nulos
     db.futbolistaspremierleague.find( { "NombreFutbolista": { $regex: /^R/i} } )// Find con regex de jugadores que empiezan por la R sin distinguir mayúsculas o minúsculas.
     db.futbolistaspremierleague.find({"altura": {$gte: 1.78 }})// Find con jugadores que pasan el 1.78
     db.futbolistaspremierleague.find( { "país": { $eq: "Inglaterra"} }) //Find para que salgan los jugadores ingleses
